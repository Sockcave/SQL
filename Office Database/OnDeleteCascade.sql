...
-- ON DELETE: OKAY to use on foreign keys NOT PRIMARY KEYS.

-- ON DELETE Example: If emp_id gets deleted, mgr_id gets set to NULL in branch table.
-- Okay to use on mgr_id since it's a FOREIGN KEY and not a primary key.

-- ON DELETE CASCADE Example: if the branch_id in branch_supplier gets deleted, the ENTIRE ROW IS DELETED.
-- branch_id  is a part of the PRIMARY KEY, and we CANNOT SET OUR PRIAMRY KEY TO NULL.